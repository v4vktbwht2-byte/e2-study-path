# バックログ

`planning/BACKLOG.md` は製品スコープ一覧、本ファイルは現在の実装順と未検証事項を管理する。

## 残タスク（優先度順）

1. [高] Phase 04: 単語集中・適応復習 — 中心学習機能を完成。
2. [高] Phase 05: 今日のプラン・滞留救済 — 毎日の導線を統合。
3. [高] Phase 06: 技能別練習・短縮模試 — 全技能の縦断動作を完成。
4. [高] Phase 07: PWA・オフライン・バックアップ — オフライン利用とデータ所有権を完成。
5. [高] Phase 08: 記録・設定・アクセシビリティ — 実用時の状態と操作性を仕上げ。
6. [高] Phase 09: 全テスト・CI・静的デプロイ — 再現可能な品質ゲートを完成。
7. [高] Phase 10: 最終監査・Pilot Release — 全受入条件と文書を確定。
8. [高] npm依存のhigh severity advisory 2件を詳細監査 — 外部照会制限の解除または明示承認後に確認。
9. [中] iPhone Safari／ホーム画面PWA実機確認 — Windows環境外のため実装後に利用者確認が必要。
10. [中] 主要フローのスクリーンリーダー手動確認 — 実機・支援技術での最終確認が必要。
11. [低] Web Speechの声質・発音確認 — 利用端末の音声エンジン依存。

## 完了

- 2026-07-27: Phase 00の資料・環境・依存互換性・Git基準点監査を完了。
- 2026-07-27: Phase 01のReact/Vite基盤、全ルート、共通UI、品質スクリプトを実装し、全Phaseゲートを完了。
- 2026-07-27: Phase 02の復習・習熟度・診断・日次計画domain、Dexie永続化、原子的回答確定、教材検証・seed・起動処理を実装し、144テストと品質ゲートを完了。
- 2026-07-27: Phase 03のオンボーディング、適応診断、Stageマップ、31レッスン・155問、回答保存・中断再開・原子的完了を実装し、205テストとPhase E2E 6件を完了。

## 見送り・保留

- 公開先の最終決定と実デプロイ（GitHub認証・Pages設定が必要）。
- ソフトウェアライセンスの追加（リポジトリ所有者の選択待ち）。
